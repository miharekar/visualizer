...
class Airtable::ShotSyncTest < ActiveSupport::TestCase
  ...
  test "it can delete a record" do
    ...
    delete = Airtable::ShotSync.new(user).delete(airtable_id)
  ...
  test "it can download new data for existing records from airtable" do
    ...
    records = Airtable::ShotSync.new(user).download
  ...
  test "it uploads a new record to airtable" do
    ...
    user = users(:miha)
    identity = user.identities.first
    sync = Airtable::ShotSync.new(user)
