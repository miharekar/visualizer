...
- Coffee, grinder, tags, and notes open a server-rendered editor with Save/Cancel.
  Existing combobox, Tagify, and rich-text widgets are reused.
...
- Search targets the results frame and updates browser history. Premium users
  get debounced search; free users submit Search. Notes drafts sit outside results
  so searching does not discard them. Opening another editor replaces the draft.
...
- Columns have a separate controller. Drag to reorder, check visibility, then
  Apply. Apply reloads results, clearing selection and loaded pages. Reset stores
  SQL NULL and restores defaults.
...
- `GET /shots/journal/edit`: renders one field editor for owned selected shots;
  no selection returns an empty editor frame for Cancel.
