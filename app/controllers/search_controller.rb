...
class SearchController < ApplicationController
  ...
  def index
    @shots = Shot.active.visible.with_avatars.by_start_time
  ...
  def unique_values_for(filter)
    Rails.cache.fetch("#{Shot.active.visible.cache_key_with_version}/#{filter}") do
      if filter == :user
        User.visible.by_name
      else
        Shot.active.visible.distinct.pluck(filter).compact.map(&:strip).uniq(&:downcase).sort_by(&:downcase)
      end
    end
  end
end
