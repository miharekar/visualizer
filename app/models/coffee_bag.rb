class CoffeeBag < ApplicationRecord
  ...
  def duplicate(roast_date)
    dup.tap do |copy|
      ...
      copy.notes = notes.body if notes.body.present?
    end
  end

  def to_api_json
    attribute_names = CoffeeBag::DISPLAY_ATTRIBUTES + %w[id roaster_id canonical_coffee_bag_id name roast_date frozen_date defrosted_date url archived_at]
    attributes.slice(*attribute_names).tap do |json|
      json["notes"] = rich_text_html(:notes).presence
